...
- [ ] 운영 도메인에서만 `/directions` 지도가 보이지 않는 오류를 preview와 비교 재현하고 코드 원인일 때만 최소 수정·회귀 테스트·운영 검증 — 원인은 Maps Proxy가 동적으로 로드하는 `maps.googleapis.com`·`maps.gstatic.com` SDK 모듈이 CSP `script-src`에 빠져 차단된 것. `script-src`에 두 공식 host를 최소 추가하고 vector renderer `.gm-style`도 성공으로 인정했으며, Maps Proxy 실패 시 Google Maps 공유 UI가 제공한 keyless `https://www.google.com/maps/embed?pb=...` iframe으로 전환하도록 보완. local `/directions` fresh navigation에서 지도 타일·지도/위성 컨트롤·확대/축소·Street View 컨트롤이 실제 렌더링되는 것을 확인. focused 22/22, TypeScript·ESLint error 0, DB 없는 Unit 107 files/1,560 tests, production build 29.08s 통과. checkpoint `15676c71` 저장 후 실제 `star-pibu.com/directions`를 cache-buster로 재확인했으나 여전히 이전 빈 지도 화면을 제공하여 custom domain이 최신 revision을 반영하지 않는 기존 platform/edge 배포 불일치 blocker가 재현됨
- [x] 초기 접속에서 raw text가 먼저 나타난 뒤 hero가 표시되는 FOUC를 재현하고 CSS 준비 신호·안전 timeout·운영 custom domain의 최신 HTML 반영 상태를 분리 점검한 뒤 코드 원인만 최소 보완 — 4초 unconditional 해제를 제거하고 production CSS는 두 animation frame 뒤에, Vite 개발 환경은 `style[data-vite-dev-id$="/client/src/index.css"]` 감지 뒤에만 gate를 해제하도록 보완. CSS load error는 접근성 fail-safe로 처리. local fresh navigation은 navy background only → 정상 hero, 운영 `star-pibu.com` fresh navigation도 정상 hero·레이아웃·텍스트로 전환되어 raw text flash가 재현되지 않음. FOUC regression, TypeScript·ESLint error 0, DB 없는 Unit 107 files/1,560 tests, production build 31.51s 통과
- [ ] 초기 접속 지연을 운영·local에서 재측정하고 HTML·CSS·JS·이미지·폰트 병목 중 사용자 체감 영향이 가장 큰 비예약 항목 하나만 최소 수정·회귀 테스트·품질 검증 — 운영 fresh navigation에서 TTFB 4.34초, FCP 4.64초, DCL 4.69초, load 5.72초. CSS·main JS·hero image 요청은 TTFB 이후 15ms 내 완료되어 FCP의 주 병목은 43KB HTML의 edge/origin TTFB로 재확인. app code에서 LCP hero와 경쟁하던 below-fold 재생의료 배너의 high-priority preload(PC/mobile)를 제거. local fresh navigation에서 hero는 link preload로 152ms에 시작되고 banner는 콘텐츠 img 요청으로 800ms에 시작해 초기 네트워크 경쟁이 제거됨; focused 2/2, TypeScript·ESLint error 0, DB 없는 Unit 108 files/1,561 tests, production build 32.17초 통과. checkpoint `e7446fa8` 후 `star-pibu.com`은 여전히 banner를 link preload로 4,573ms에 요청하며, 관리형 `starpibu-qdq7tysk.manus.space`도 PC/mobile banner preload 2개를 계속 반환함. 따라서 최신 revision이 두 공개 도메인 모두에 반영되지 않는 platform/edge deployment blocker로 판정
- [ ] 서버 응답 대기 중 초기 HTML에서 접근 가능한 브랜드 로딩 UI를 표시하고 app CSS 준비 뒤 즉시 제거되도록 FOUC 게이트와 최소 통합·회귀 테스트·local 검증 — `role="status"`·`aria-live="polite"` 브랜드 로딩 UI와 reduced-motion 대응 spinner를 initial HTML에 추가. app CSS 준비 뒤 기존 gate 해제와 함께 loader를 제거하도록 연결했으며, focused 2/2와 local fresh navigation hero 렌더링을 확인. 최종 DOM에서 initial gate·loader는 제거되고 root visibility는 visible; 전체 품질 gate와 운영 확인 진행 중
