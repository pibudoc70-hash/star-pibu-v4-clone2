...
- [ ] Checkpoint recovery: user-approved `user_github/main` commit `a14f70e0` (Consultation concern native option accessibility)와 local SEO-1 commit `25aca4d`를 보존적으로 병합하고, 충돌이 발생하면 두 변경을 모두 유지한 뒤 full gate와 checkpoint save를 재시도. 예약·OTP·운영 DB·deploy 변경 금지
