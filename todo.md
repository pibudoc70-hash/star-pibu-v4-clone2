...
- [ ] Umami에서 `map_fallback_shown` event의 locale별 발생률을 확인하고 수집 상태를 기록 — telemetry는 최신 checkpoint부터 locale·surface만 전송하도록 적용됐으나, 이 task에는 Umami dashboard/API 접근이 없고 관찰 기간도 부족해 실제 rate 산출 보류. dashboard read-only 접근 또는 export 필요
