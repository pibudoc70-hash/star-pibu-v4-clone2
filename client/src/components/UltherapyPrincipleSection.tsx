...
type CropName = keyof typeof CROP_POSITIONS;

function IllustrationCrop({ name, className, testId }: { name: CropName; className: string; testId: string }) {
  const infographic = EQUIPMENT_EXPLANATORY_INFOGRAPHICS.울쎄라피프라임;

  return (
    <div
      aria-hidden="true"
      className={`overflow-hidden rounded-2xl border border-[#e6d9c8] bg-[#f7f1e7] ${className}`}
      data-testid={testId}
      style={{
        backgroundImage: `url(${infographic.src})`,
        backgroundPosition: CROP_POSITIONS[name],
        backgroundRepeat: "no-repeat",
        backgroundSize: name === "layers" ? "145% auto" : "620% auto",
      }}
    />
  );
}
  ...
  return (
    <section className="equipment-detail__ultherapy-principle mb-12 overflow-hidden rounded-3xl border border-[#e7ddd0] bg-[#fffdf8] shadow-[0_18px_48px_rgba(42,52,74,0.08)]" aria-labelledby="ultherapy-principle-heading">
      ...
      <div className="space-y-12 px-5 py-8 sm:px-8 sm:py-10 md:px-12 md:py-14">
        <section className="grid gap-8 lg:grid-cols-[0.9fr_1.1fr] lg:items-center" aria-labelledby="ultherapy-layer-heading">
          <div className="rounded-2xl border border-[#e6d9c8] bg-[#f7f1e7] p-5 sm:p-7">
            <IllustrationCrop
              name="layers"
              testId="ultherapy-layer-illustration"
              className="mb-6 aspect-[16/9] w-full"
            />
            <div className="mb-5 flex items-center gap-3 text-[#24324e]">
            ...
            <div className="overflow-hidden rounded-2xl border border-[#d9cbb9] bg-[#fffdf8]" aria-hidden="true">
              {ULTHERAPY_PRIME_PRINCIPLE.layers.map((layer, index) => (
                <div key={layer.depth} className={`relative flex min-h-20 items-center justify-between px-5 ${index === 0 ? "bg-[#f8ded4]" : index === 1 ? "bg-[#f0cbbd]" : "bg-[#e3b49e]"}`}>
                  <span className="font-semibold text-[#3f2c27]">{layer.name}</span>
                  <span className="rounded-full bg-white/70 px-3 py-1 text-sm font-bold text-[#24324e] shadow-sm">{layer.depth}</span>
                </div>
              ))}
            </div>
          </div>
        ...
        <section className="rounded-2xl border border-[#dfe4ea] bg-[#f4f6f8] p-5 sm:p-7" aria-labelledby="ultherapy-timeline-heading">
          ...
          <ol className="mt-7 grid gap-5 md:grid-cols-3">
            {ULTHERAPY_PRIME_PRINCIPLE.timeline.map((phase, index) => (
              <li key={phase.label} className="relative flex gap-4 rounded-xl bg-white p-5 shadow-sm md:after:absolute md:after:left-full md:after:top-1/2 md:after:h-px md:after:w-5 md:after:bg-[#c4a882] md:last:after:hidden">
                <IllustrationCrop
                  name={(["timelineOne", "timelineTwo", "timelineThree"] as const)[index]}
                  testId={`ultherapy-timeline-illustration-${index + 1}`}
