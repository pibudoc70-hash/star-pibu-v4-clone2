...
- [ ] public raw SEO deployment verification: local About prerender contract는 통과했지만 `star-pibu.com`/`www` raw response가 이전 fallback description 1·hreflang 12를 반환하고 Manus-space host는 500을 반환함. platform deployment/domain sync가 정상화된 뒤 동일 audit으로 code revision 반영을 재확인 — domain/deploy/rollback 설정 변경 금지
