...
- [ ] Logo and photo asset restoration: audit the current preview, browser console/network asset failures, and recent normal asset references to identify missing logo/photo rendering. Restore only the existing approved asset URLs and their former responsive/crop behavior; do not generate replacements, change reservation/OTP·운영 DB. Remaining blocker: `About.tsx` still references `/api/storage/medical_team_53232402.jpg`, which returns 415; no matching original was found in user uploads, downloads, static-asset storage, or the supplied backups, and prior git history contains no approved managed replacement path
