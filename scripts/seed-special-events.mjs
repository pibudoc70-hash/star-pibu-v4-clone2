/**
 * SPECIAL EVENT 샘플 데이터 DB 삽입 스크립트
 * 실행: node scripts/seed-special-events.mjs
 */
...
async function seedSpecialEvents() {
  ...
  try {
    ...
    // 기존 SPECIAL EVENT 확인
    const [existing] = await conn.execute('SELECT COUNT(*) as cnt FROM events WHERE isSpecialEvent = "1"');
    const count = existing[0].cnt;
    console.log(`기존 SPECIAL EVENT 수: ${count}`);

    if (count > 0) {
      console.log('이미 SPECIAL EVENT 데이터가 있습니다. 삽입을 건너뜁니다.');
      return;
    }
    ...
    for (const ev of specialEvents) {
      const now = new Date();
      await conn.execute(
        `INSERT INTO events (type, category, title, subtitle, \`desc\`, content, badge, tag, hot, isFeatured, cta, accent, accentDark, accentBg, iconBg, iconType, badgeColor, \`date\`, imageUrl, sortOrder, isActive, isSpecialEvent, productName, normalPrice, discountPrice, createdAt, updatedAt)
         VALUES (?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?)`,
        [
          ...
          ev.date, ev.imageUrl, ev.sortOrder, ev.isActive, ev.isSpecialEvent,
          ev.productName, ev.normalPrice, ev.discountPrice, now, now
        ]
      );
      console.log(`✓ SPECIAL EVENT 삽입: ${ev.title}`);
    }

    console.log(`\n총 ${specialEvents.length}개 SPECIAL EVENT 삽입 완료`);
  } catch (err) {
