...
/**
 * 시술 payload 공통 필드 스키마.
 * create(required 포함)와 update(optional 전체) 모두 이 shape를 기반으로 사용한다.
 */
const treatmentPayloadShape = {
...
export const treatmentsRouter = router({
  ...
  // 관리자: 시술 생성
  // 필수 필드(categoryId, name, nameEn, desc, time, recovery)는 create에서만 required로 오버라이드
  create: adminProcedure
    .input(z.object({
      ...treatmentPayloadShape,
      categoryId: z.string(),
