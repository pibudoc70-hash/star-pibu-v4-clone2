...
- [ ] 배포된 Equipment2·Equipment3 detail의 오류·not-found native link 화면을 실제로 확인하고, GitHub 원격 백업 상태를 점검 — GitHub `user_github/main`은 local `f621eace`와 일치 확인. 다만 custom domain Equipment3 존재하지 않는 slug에서 DOM `BUTTON`(href 없음)이 관찰됨. custom HTML은 cache-bust/no-cache에도 이전 entry `/assets/index-CYu0gIb4.js`를 제공하며, 최신 local build entry `/assets/index-DnJU5HUJ.js` 요청도 asset이 아닌 HTML fallback 200을 반환. 프로젝트 도메인은 동일 경로에서 500을 반환하여 최신 배포 비교 불가; platform domain-to-revision 반영 상태 확인 필요
- [ ] 비예약 영역의 다음 접근성 개선 후보를 한 건만 선정해 test-first로 구현·검증
