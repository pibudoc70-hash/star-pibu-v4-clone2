...
- [ ] 공개 홈 fresh navigation은 첫 capture에서 스타일 적용 전 raw text가 보였으나, 약 17초 후 final hero는 정상 렌더링됨. FOUC gate의 실제 시작 타이밍과 CSS ready 신호는 감사 우선 항목으로 재평가
- [ ] 홈 첫 진입에서 facility·YouTube·FAQ·notices 등 아래 폴드 section의 actual lazy loader가 즉시 실행되지 않도록 기존 fallback 높이를 유지하는 DeferredMount를 최소 도입하고, LCP hero·anchor scroll·다국어 route에 영향을 주지 않는지 검증 — Management Devices·Philosophy·Results·YouTube·FAQ·Notices를 400px rootMargin DeferredMount로 감싸 loader가 viewport 근처에서만 실행되도록 구현. focused 4/4, TypeScript·ESLint error 0, DB 없는 Unit 113 files/1,570 tests 통과. browser local hero 정상 렌더링 확인. production build는 6,785 modules 변환 뒤 rendering chunks 중 sandbox 외부 SIGTERM 143으로 두 차례 종료되어 CI 또는 별도 승인 전에는 완료 판정 보류
