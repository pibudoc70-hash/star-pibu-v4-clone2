...
- [x] SPECIAL EVENT desktop lead/compact layout: redesigned only md-and-up from the equal three-card grid to first-event lead card plus remaining compact inline-expand price rows, capped at six total events and five compact rows before show-more. Added `lead`/`compact`/default `legacy` EventCard variants; legacy preserves the prior mobile card and link behavior, while desktop lead retains 3:2 hoverable media and a quiet CTA, and compact rows are image-free, keyboard-accessible inline disclosures. Moved `VAT 포함` to a single desktop section-level label, left-aligned the existing header, removed the decorative divider, and updated the desktop skeleton to one lead card plus compact rows. EventTableMobile, reservation/OTP/booking paths, Header/Hero/Footer/MobileBottomCTA, PainManagementGuide placement, Home SEO, event source data/CTAs/links, `useChatConfig`, `externalBooking`, packages, domains, and operational DB are unchanged. regression-first 6 fail→focused 13 pass; TypeScript pass; ESLint 0 errors/105 existing warnings; DB-free unit 198 files/1,844 tests pass. Production build generated existing SEO JSON then was externally terminated with SIGTERM 143 in early Vite transformation, with no source error. Automated screenshot capture failed; no lazy-loading behavior was changed
