...
- [ ] 운영 도메인에서만 `/directions` 지도가 보이지 않는 오류를 preview와 비교 재현하고 코드 원인일 때만 최소 수정·회귀 테스트·운영 검증 — Maps Proxy SDK의 CSP 허용·vector renderer 판정·공유형 Google Maps iframe fallback을 보완했고 local 지도 타일·컨트롤 렌더링을 확인. 공개 도메인은 최신 revision 미반영으로 이전 빈 지도 화면을 제공하는 platform/edge blocker가 남음
...
- [ ] 초기 접속 지연을 운영·local에서 재측정하고 HTML·CSS·JS·이미지·폰트 병목 중 사용자 체감 영향이 가장 큰 비예약 항목 하나만 최소 수정·회귀 테스트·품질 검증 — below-fold 배너 preload를 제거해 hero 요청을 우선화했으나, 공개 도메인이 이전 HTML을 제공하는 platform/edge blocker가 남음
- [ ] 서버 응답 대기 중 초기 HTML에서 접근 가능한 브랜드 로딩 UI를 표시하고 app CSS 준비 뒤 즉시 제거되도록 FOUC 게이트와 최소 통합·회귀 테스트·local 검증 — loading UI는 local에서 검증됐으나 공개 도메인이 이전 HTML을 제공하는 platform/edge blocker가 남음
- [ ] 공개·관리형 도메인이 최신 저장 revision을 제공하도록 cache/revision 연결 상태를 확인하고, 안전하게 가능한 cache invalidation 또는 연결 조치 후 최신 loading UI·성능 preload·지도 CSP 응답을 재검증
- [ ] My Browser로 Manus 관리 화면의 `project/QdQ7tySKssCV8bdRzPPxg4` 접근을 시도했으나 `[internal] record not found`로 대상 revision/cache 제어 화면이 열리지 않아 cache invalidation 또는 revision 연결 조치를 수행할 수 없음. 접근 권한 또는 프로젝트 레코드 매핑 복구가 필요
