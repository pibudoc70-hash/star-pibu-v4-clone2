...
import { ULTHERAPY_PRIME_PRINCIPLE, isUltherapyPrimeSlug } from "../../shared/ultherapyPrinciple";
...
export function buildEquipmentPrerenderedHtml(template: string, item: Equipment3Item, lang: Lang, pathName: string): string {
  ...
  const ultherapyPrincipleBody = lang === "ko" && isUltherapyPrimeSlug(item.slug)
    ? `<section aria-labelledby="ultherapy-principle-heading"><h2 id="ultherapy-principle-heading">${escapeHtml(ULTHERAPY_PRIME_PRINCIPLE.heading)}</h2><p>${escapeHtml(ULTHERAPY_PRIME_PRINCIPLE.subtitle)}</p><section><h3>${escapeHtml(ULTHERAPY_PRIME_PRINCIPLE.layerHeading)}</h3><dl>${ULTHERAPY_PRIME_PRINCIPLE.layers.map((layer) => `<dt>${escapeHtml(`${layer.depth} · ${layer.name}`)}</dt><dd>${escapeHtml(layer.description)}</dd>`).join("")}</dl></section><section><h3>${escapeHtml(ULTHERAPY_PRIME_PRINCIPLE.principleHeading)}</h3><ol>${ULTHERAPY_PRIME_PRINCIPLE.steps.map((step) => `<li><h4>${escapeHtml(step.title)}</h4><p>${escapeHtml(step.description)}</p></li>`).join("")}</ol></section><section><h3>${escapeHtml(ULTHERAPY_PRIME_PRINCIPLE.timelineHeading)}</h3><ol>${ULTHERAPY_PRIME_PRINCIPLE.timeline.map((phase) => `<li><h4>${escapeHtml(phase.label)}</h4><p>${escapeHtml(phase.description)}</p></li>`).join("")}</ol></section><aside>${escapeHtml(ULTHERAPY_PRIME_PRINCIPLE.notice)}</aside></section>`
    : "";
  const body = `<main id="crawler-content" lang="${lang}"><article><header><h1>${escapeHtml(name)}</h1><p>${escapeHtml(localized(item, "desc", lang))}</p></header><section><h2>${escapeHtml(text.overview)}</h2><table><tbody>${rows.map(([label, value]) => `<tr><th scope="row">${escapeHtml(label)}</th><td>${escapeHtml(value)}</td></tr>`).join("\n")}</tbody></table></section>${ultherapyPrincipleBody}${faqBody}${quoteBody}<footer><p>부산 서면 스타피부과 · 부산광역시 부산진구 서면로 74 아이온시티빌딩 4층 · 051-818-2300</p></footer></article></main>`;
