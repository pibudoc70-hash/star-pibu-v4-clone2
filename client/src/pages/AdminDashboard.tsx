...
export default function AdminDashboard() {
  ...
  const [reservationNotes, setReservationNotes] = useState<Record<number, string>>({}); // 예약별 메모 상태
  ...
  const handleStatusChange = (reservationId: number, newStatus: ReservationStatus) => {
    const adminNote = reservationNotes[reservationId] || "";
    updateReservationStatusMutation.mutate({ id: reservationId, status: newStatus, adminNote });
  };

  const handleAdminNoteChange = (reservationId: number, note: string) => {
                    ...
                    <table className="w-full text-sm">
                      <thead>
                        <tr className="border-b border-[#F3F4F6]" style={{ background: "#F9FAFB" }}>
                          ...
                          <th className="text-left px-6 py-3 text-xs font-semibold text-[#6B7280] uppercase tracking-wider">추가사항</th>
