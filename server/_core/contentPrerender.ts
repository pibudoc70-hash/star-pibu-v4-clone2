...
import { getNoticeById } from "../db/notices";
...
export function buildNoticePrerenderedHtml(template: string, notice: Notice, pathname: string): string {
  ...
  const article = {
    ...
    author: { "@type": "Organization", name: "스타피부과", url: BASE_URL }, publisher: { "@type": "MedicalClinic", name: "스타피부과", url: BASE_URL },
  };
  const jsonLd = JSON.stringify(article).replace(/</g, "\\u003c");
  const body = `<main id="crawler-content"><article><header><h1>${escapeHtml(notice.title)}</h1><time datetime="${toIso(notice.createdAt)}">${notice.createdAt.toISOString().slice(0, 10)}</time></header><div>${escapeHtml(notice.content).replace(/\n/g, "<br />")}</div></article></main>`;
  return template
...
export function buildResearchPrerenderedHtml(template: string, pathname: string): string {
  ...
  const article = {
    ...
    publisher: { "@type": "MedicalClinic", name: "스타피부과", url: BASE_URL },
  };
  const papers = [
  ...
  const body = `<main id="crawler-content"><article><header><h1>${escapeHtml(title)}</h1><p>${escapeHtml(description)}</p><time datetime="${RESEARCH_PUBLISHED}">${RESEARCH_PUBLISHED}</time></header><section><h2>주요 연구 논문</h2><ul>${papers.map((paper) => `<li>${escapeHtml(paper)}</li>`).join("")}</ul></section></article></main>`;
  return template
...
export function registerContentPrerender(app: Express): void {
  ...
  app.get(["/notice/:id", "/:lang/notice/:id"], async (req: Request, res: Response, next: NextFunction) => {
    ...
    try {
      ...
      res.setHeader("Content-Type", "text/html; charset=utf-8");
      res.setHeader("Cache-Control", "no-cache, must-revalidate");
      res.send(buildNoticePrerenderedHtml(template, notice, req.path));
    } catch (error) {
