...
- [ ] 1단계: Directions 지도 복구와 로드 실패 대체 UI 구현 — 5개 언어·주소 복사·길찾기·키보드 접근·실제 운영 화면 검증 후 체크포인트 저장
- [ ] 2단계: 주소·전화·진료·교통 정보의 공통 원본 정합화 — Footer·Directions·MedicalClinic JSON-LD 일치 검증 후 체크포인트 저장
- [ ] 3단계: Equipment3 상세 초기 로딩 경험 개선 — 기존 본문·FAQ·영상·외부 네이버/카카오 CTA 보존 회귀 검증 후 체크포인트 저장
- [ ] 4단계: 홈·상세 프리렌더 응답 성능 계측 및 안전한 캐시 정책 개선 — 원본 HTML·SEO와 운영 응답 검증 후 체크포인트 저장
- [ ] 5단계: 프리렌더·클라이언트 구조화 데이터의 엔터티·필드·중복 정합성 강화 — 원본 HTML SEO 회귀 검증 후 체크포인트 저장
- [ ] 6단계: 공개·관리자 비예약 화면의 접근성 경고 우선 정리 — 예약/OTP 파일 제외, 키보드·label·상호작용 회귀 검증 후 체크포인트 저장
