...
- [ ] 운영 도메인에서만 `/directions` 지도가 보이지 않는 오류를 preview와 비교 재현하고 코드 원인일 때만 최소 수정·회귀 테스트·운영 검증 — 2026-08-17 운영과 local preview 모두 본문은 렌더링되나 동일한 811×400 `https://maps.google.com/maps?...&output=embed&hl=ko` fallback iframe의 타일이 빈 회색 영역으로 표시됨. Maps Proxy script와 `window.google.maps.Map`은 정상 로드됐지만 MapView가 이미지·canvas만 지도 성공으로 판정해 vector renderer의 `.gm-style` root를 놓치고 8초 뒤 broken iframe으로 잘못 전환할 가능성을 확인. `.gm-style`도 성공으로 인정하도록 최소 수정·focused 4/4 통과했으나 local 9초 DOM 재검증에서는 `.gm-style` 없이 iframe fallback이
여전히 발생해 Maps Proxy 실제 생성 실패와 invalid iframe fallback을 분리 추가 진단 중. Google Maps 직접 페이지는 같은 좌표에서 정상 타일을 렌더링하며, 공식 Embed API는 API key를 요구하므로 current `output=embed` URL을 일반 Maps 경로로 교체할 수 없음. Google Maps 공유 UI의 ‘지도 삽입’에서 받은 API key 없는 `https://www.google.com/maps/embed?pb=...` URL은 iframe 내부에서 같은 좌표의 타일을 실제 렌더링함을 확인. 이 공식 shared embed URL로 fallback을 교체하고 referrer policy를 권장 값으로 강화했으나 local Directions final screenshot은 여전히 빈 회색 영역이어서 actual iframe src·response와 MapView fallback DOM을 추가 점검 중
...
- [x] 번역 처리 한도가 회복되면 zh-TW FAQ 72개 상세 페이지·279문항을 원문 zh/ko 기준으로 대만 번체 현지화하고, 데이터 저장 전 누락·간체 혼입·원문 보존·페이지 렌더링을 전수 검증 — 재시도 요청 후 읽기 전용 DB 전수 집계에서 FAQ 원문 72개 상세·279문항과 `faqsZhTw` 72개 상세·279문항이 이미 모두 일치함을 확인. 표본 5개 행의 첫 질문도 대만 번체로 저장되어 있어 원문·간체·다른 언어 데이터 변경 없이 재번역을 중단
- [x] 홈 FAQ 탭과 선택된 FAQ 질문의 ‘울쎄라’ 표기를 ‘울쎄라피 프라임’으로 최소 변경하고 다른 시술명·FAQ 데이터·레이아웃을 보존한 채 회귀 테스트·local 검증 — FAQ 탭, 선택 질문, 관련 써마지 FAQ의 비교 표기를 일관되게 갱신. local 홈에서 ‘울쎄라피 프라임’ 탭과 ‘울쎄라피 프라임은 어떤 시술인가요?’ 질문이 렌더링되고 다른 FAQ 탭·레이아웃은 유지됨. home prerender focused 4/4, TypeScript·ESLint error 0, DB 없는 Unit 107 files/1,560 tests, production build 32.19초 통과
- [x] 피부과 전문의 탭에서 우혜진·이기욱 선택 시 조시형 탭보다 큰 상단·하단 여백이 생기는 원인을 재현하고, 콘텐츠 높이·이미지·transition 레이아웃을 최소 수정해 탭별 정렬을 일관되게 보정 — 원인은 active 의사의 콘텐츠 길이에 따라 desktop panel 전체 높이가 741px(조시형) 또는 624px(우혜진·이기욱)으로 달라지는 구조. 모든 탭이 조시형 기준 최소 741px을 유지하도록 CSS를 보완. fresh navigation 후 세 탭 모두 computed min-height·actual height 741px, 동일 top 위치를 확인. focused 1/1, TypeScript·ESLint error 0, DB 없는 Unit 108 files/1,561 tests, production build 28.19초 통과
- [x] 모바일 전문의 슬라이드에서 우혜진·이기욱 선택 시 조시형과 다른 과도한 여백을 재현하고, 이미지 viewport·track height·inactive panel 높이의 최소 보완으로 탭 전환 레이아웃을 일관되게 조정 — 흰색 가운이 많은 우혜진·이기욱 사진에서 100px white fade가 빈 여백처럼 보이는 원인을 확인. fade를 56px로 축소하고 panel background와 일치하도록 보완했으며, fresh navigation 후 실제 computed height 56px와 panel background gradient 적용을 확인. focused 2/2, TypeScript·ESLint error 0, DB 없는 Unit 108 files/1,562 tests, production build 28.91초 통과
