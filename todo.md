...
- [ ] If a future field PageSpeed run still requires Lighthouse `priorityHinted` to pass, separately evaluate whether a stable non-redirect asset endpoint can replace the Hero mobile-logo managed-storage URL without breaking the existing storage, cache, asset identity, deployment, or SEO contracts. Do not implement without approval.
