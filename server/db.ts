...
import { InsertUser, users, events, popupEvents, reservations, guestOtps, treatments, treatmentCategories, unavailableSlots, youtubeVideos, equipment3 } from "../drizzle/schema";
import type { InsertEvent, InsertReservation, InsertTreatment, InsertTreatmentCategory, Treatment, TreatmentCategory, UnavailableSlot, InsertUnavailableSlot, YouTubeVideo, InsertYouTubeVideo, Equipment3Item, InsertEquipment3Item } from "../drizzle/schema";
...
export async function getEquipment3List(): Promise<Equipment3Item[]> {
  ...
  return db
    .select()
    .from(equipment3)
    .where(eq(equipment3.isActive, "1"))
    .orderBy(asc(equipment3.sortOrder));
}
 ...
 */
export async function reorderEquipment3Items(
