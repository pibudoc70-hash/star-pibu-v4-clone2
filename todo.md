...
- [ ] 초기 접속에서 raw text가 먼저 나타난 뒤 hero가 표시되는 FOUC를 재현하고 CSS 준비 신호·안전 timeout·운영 custom domain의 최신 HTML 반영 상태를 분리 점검한 뒤 코드 원인만 최소 보완 — 운영 홈 fresh navigation에서 raw text가 실제 노출됨. 해당 시점에 compiled stylesheet `assets/index-Di2BLywU.css`는 `document.styleSheets`에 존재하지만 initial gate는 제거되고 root가 visible이므로, 4초 unconditional timeout 또는 stylesheet 등록만으로 성공 판정하는 로직이 CSS 적용 완료보다 먼저 gate를 해제하는지 조사 중. 4초 해제를 제거하고 production CSS는 두 animation frame 뒤에 gate를 해제하며, Vite 개발 환경의 `style[data-vite-dev-id$="/client/src/index.css"]`도 별도 감지하도록 보완. local fresh navigation 첫 capture는 navy background only, 이어지는 capture는 정상 hero로 전환되어 raw text flash가 재현되지 않음; 전체 품질 gate와 운영 반영 확인 진행 중
