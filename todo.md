...
- [ ] SEO-3 Search Console·네이버 서치어드바이저 색인 상태 점검: available connector·로그인 session·public endpoints만 사용해 index coverage, sitemap, query/page performance data를 수집. 권한 또는 verified property가 없으면 데이터 접근 없이 필요한 사용자 조치를 기록하며, secret·site ownership·운영 DB 변경 금지
- [ ] SEO-4 울쎄라 legacy URL ownership·redirect 감사: `/treatment/:name`, `ulthera-classic`, `ulthera` 및 legacy Korean names의 canonical ownership·current client navigation·crawl behavior를 대조. content identity가 일치할 때만 server-side permanent redirect를 regression-first로 추가하고, classic/prime 혼동이나 예약/OTP 변경은 금지
