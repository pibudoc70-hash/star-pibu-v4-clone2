...
describe("index SEO fallback metadata", () => {
  it("클라이언트 실행 전에도 홈 title과 keywords를 사용자 지정 값으로 제공한다", () => {
    ...
    expect(indexHtml).toContain(`<meta data-rh="true" data-seo-fallback="home" name="keywords" content="${homeKeywords}" />`);
  });

  it("소셜 미리보기 title도 홈 SEO title과 일치시킨다", () => {
    expect(indexHtml).toContain(`<meta data-rh="true" data-seo-fallback="home" property="og:title" content="${homeTitle}" />`);
