...
function CategoryBadge({ category, labelIntl, labelDomestic }: {
  ...
  return (
    <span className="inline-block text-xs font-semibold px-2 py-0.5 rounded-full bg-[var(--color-gold-pale)] text-[var(--color-gold-dark)]">
      {labelDomestic}
    </span>
  );
}
...
export default function Research() {
  ...
  return (
    <MainLayout>
      ...
      {/* 페이지 히어로 */}
      <section
        className="dr-page-header py-16 md:py-24"
      >
        <div className="container mx-auto px-4 text-center">
          ...
          <h1 className="dr-page-header-title text-3xl md:text-5xl font-bold mb-4">
            {rp.heroTitle}
          </h1>
          <p className="dr-page-header-tagline text-base md:text-lg max-w-2xl mx-auto leading-relaxed">
            {rp.heroDesc}
          </p>
          {/* 통계 요약 */}
          <div className="flex flex-wrap justify-center gap-8 mt-10">
            {[
              ...
              <div key={stat.label} className="text-center">
                ...
                <p className="text-[var(--brand-text-mid)] text-sm mt-1">{stat.label}</p>
              </div>
            ))}
          </div>
          {/* PubMed 저자 검색 버튼 */}
              ...
              >
                ...
                {paper.pmid && (
                  <a
                    ...
                    target="_blank"
                    rel="noopener noreferrer"
                          className="inline-flex items-center gap-1 mt-3 text-xs font-semibold text-[var(--color-gold-dark)] hover:text-[var(--color-gold-primary)] transition-colors"
          ...
          <ol className="space-y-6" aria-label={rp.domesticJournalTitle}>
            {domPapers.map((paper) => (
              ...
              >
                ...
                <p className="text-sm font-semibold text-[var(--color-gold-dark)] mb-1">{paper.journal}</p>
                {"detail" in paper && paper.detail && (
                  <p className="text-sm text-gray-600 leading-relaxed">{paper.detail}</p>
                )}
              </li>
